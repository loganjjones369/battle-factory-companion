...
function speedOf(pokemon, levelNumber, round) {
  return getEffectiveSpeed(getStats(pokemon, pokemon, levelNumber, round), 'healthy');
}

export function buildDraftSetSpread({ draft = [], levelMode = 'Open Level', battle = 1, swaps = 0, weather = 'none' } = {}) {
  ...
  if (!pool.supported) {
    return { supported: false, reason: pool.reason, rows: [] };
  }

  const rows = draft.filter(Boolean).map((draftEntry, index) => {
    const bucket = index < pool.elevation ? pool.upgradeBucket : pool.baseBucket;
    const possibleSets = [...(index < pool.elevation ? pool.upgradedPool : pool.regularPool)]
      .filter((set) => norm(set.species) === norm(draftEntry.species));
    ...
    const targets = draft.filter((target, targetIndex) => target && targetIndex !== index).map((target) => {
      ...
      const setResults = possibleSets.map((candidate) => {
        ...
        const attackerSpeed = speedOf(attacker, levelNumber, round);
        const defenderSpeed = speedOf(targetSet, levelNumber, round);
        return {
          set: attacker,
          best,
          faster: attackerSpeed > defenderSpeed,
          speed: attackerSpeed,
          targetSpeed: defenderSpeed,
        };
      });
      ...
      const guaranteedSpeed = setResults.filter((x) => x.faster);

      return {
        target,
        total: setResults.length,
        meaningful: meaningful.length,
        twoHit: twoHit.length,
        fiftyPlus: fiftyPlus.length,
        faster: guaranteedSpeed.length,
        sets: setResults,
      };
    });
    ...
    return {
      ...
      slotIndex: index,
      bucket,
      elevated: index < pool.elevation,
      possibleSets,
      setCount: possibleSets.length,
      targets,
      pressureTargets,
      pressureScore,
    };
