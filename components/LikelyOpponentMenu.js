  ...
  return <>
    <View style={styles.card}>
      <Pressable onPress={() => setExpanded((value) => !value)} style={styles.compact}>
        ...
        <View style={styles.topRow}>{top.map((group) => <View key={norm(group.species)} style={styles.topPokemon}><View style={styles.spriteCircle}><Image source={{ uri: `${SPRITES}${spriteIdForSpecies(group.species)}.png` }} style={styles.sprite} /></View><Text numberOfLines={1} style={styles.name}>{group.species}</Text></View>)}</View>
        <Text style={styles.chevron}>{expanded ? '▲' : '▼'}</Text>
      </Pressable>
      {expanded && <View style={styles.expanded}>
        <Text style={styles.title}>MOST LIKELY REMAINING</Text>
        <Text style={styles.note}>Share = surviving candidate-set frequency. It is a ranking aid, not a guaranteed in-game probability.</Text>
        <ScrollView style={styles.list} nestedScrollEnabled>{likely.map((group) => <Pressable key={norm(group.species)} onPress={() => setSelectedSet(group.sets[0])} style={styles.row}>
