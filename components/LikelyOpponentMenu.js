...
import { analyzeCandidateSwitchIns, summarizeCandidateSwitchIns } from '../data/responseAnalysis';
...
export default function LikelyOpponentMenu({ draft = [], scientist = {}, levelMode = 'Open Level', battle = 1, blockedSpecies = [], observations = [], currentTeam = [], previousOpponent = [], noland = false }) {
  ...
  const selectedCandidates = selectedGroup?.sets?.map((entry) => entry.set).filter(Boolean) || [];
  const level = levelMode === 'Open Level' ? 100 : 50;
  const round = Math.max(1, Math.ceil((Number(battle) || 1) / 7));
  const switchRows = useMemo(() => summarizeCandidateSwitchIns(analyzeCandidateSwitchIns(selectedCandidates, currentTeam, level, round)), [selectedCandidates, currentTeam, level, round]);
  const detailSet = selectedSet?.set;
  const detailPokemon = detailSet ? getPokemon(detailSet.species) : null;
  const detail = detailSet && detailPokemon?.baseStats && detailSet?.evs ? getStats(detailPokemon, detailSet, Number(detailSet?.level || level), round) : null;
  ...
  return <>
    ...
    <Modal visible={!!selectedSpecies && !selectedSet} transparent animationType="fade" onRequestClose={() => setSelectedSpecies(null)}>
      <Pressable style={styles.modalBackdrop} onPress={() => setSelectedSpecies(null)}><Pressable style={styles.modal} onPress={() => {}}>
        ...
        {!!currentTeam.length && !!switchRows.length && <View style={styles.switchSummary}><Text style={styles.switchTitle}>SWITCH-IN SNAPSHOT</Text><Text style={styles.switchSub}>Across every surviving {selectedGroup?.species} set:</Text>{switchRows.map((row) => <View key={row.species} style={styles.switchRow}><View style={{ flex: 1 }}><Text style={styles.switchName}>{row.species}</Text><Text style={styles.switchDamage}>Incoming damage: {row.minPercent.toFixed(1)}–{row.maxPercent.toFixed(1)}% across {row.candidateCount} set{row.candidateCount === 1 ? '' : 's'}</Text><Text style={styles.switchSafe}>{row.safeCount}/{row.candidateCount} sets are under 50% on switch-in</Text></View><Text style={styles.switchShare}>{percent(row.safeShare * 100)}</Text></View>)}</View>}
