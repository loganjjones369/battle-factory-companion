      ...
      <View style={styles.modal}>
        <View style={styles.header}>
          ...
          <View style={styles.headerCopy}>
            <Text style={styles.kicker}>THE FACTORY SCIENTIST</Text>
            <Text style={styles.title}>LAB ASSISTANT</Text>
            <Text style={styles.subtitle}>Your little research partner</Text>
          </View>
        ...
        <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
          ...
          {briefing.focus?.threat && analysisTeam.length > 0 && (
            <View style={styles.numbersBox}>
              ...
              {briefing.focus.threat.details.map((detail, index) => <View key={`${detail.defender?.species}-${index}`} style={styles.numberRow}>
                <Text style={styles.numberName}>{setLabel(detail.defender)}</Text>
                <Text style={styles.numberText}>{detail.aSpeed > detail.dSpeed ? `Faster: ${detail.aSpeed} vs ${detail.dSpeed}` : detail.aSpeed === detail.dSpeed ? `Speed tie: ${detail.aSpeed}` : `Slower: ${detail.aSpeed} vs ${detail.dSpeed}`}{detail.best ? `  •  ${detail.best.moveName}: ${detail.best.percentMin.toFixed(1)}%–${detail.best.percentMax.toFixed(1)}%` : ''}</Text>
              </View>)}
            </View>
          )}

          {!!briefing.alternatives?.length && <View style={styles.section}>
...
const styles = StyleSheet.create({
  ...
  scientistNote: { backgroundColor: '#cfdacb', borderWidth: 2, borderColor: '#586957', borderRadius: 12, padding: 11, marginBottom: 10 },
  noteTop: { flexDirection: 'row', justifyContent: 'space-between' },
  noteLabel: { color: '#4d5d4d', fontSize: 9, fontWeight: '900', letterSpacing: 1 }, noteSpark: { color: '#65755f' },
  actualPhrase: { color: '#29352d', fontSize: 15, lineHeight: 21, fontWeight: '900', marginTop: 5 },
  noteHint: { color: '#637064', fontSize: 9, lineHeight: 13, marginTop: 5 },
  ...
  speechLabel: { color: '#687469', fontSize: 9, fontWeight: '900', letterSpacing: 1 }, headline: { color: '#263027', fontSize: 17, fontWeight: '900', lineHeight: 23, marginTop: 5 },
  section: { backgroundColor: '#d9e1d5', borderRadius: 12, padding: 11, marginTop: 8 }, sectionTitle: { color: '#425144', fontSize: 10, fontWeight: '900', letterSpacing: 1 },
  reasonRow: { flexDirection: 'row', marginTop: 9 }, reasonIcon: { width: 22, height: 22, borderRadius: 11, backgroundColor: '#806333', color: '#fff', textAlign: 'center', lineHeight: 22, fontWeight: '900', marginRight: 8 }, reason: { flex: 1, color: '#485349', fontSize: 12, lineHeight: 17, fontWeight: '700' },
  ...
  watchBox: { backgroundColor: '#fff4cf', borderWidth: 2, borderColor: '#8a7747', borderRadius: 12, padding: 11, marginTop: 8 }, watchText: { color: '#5b512f', fontSize: 12, lineHeight: 17, marginTop: 5, fontWeight: '700' },
  footer: { color: '#737a72', fontSize: 9, lineHeight: 14, margin: 10, textAlign: 'center' }, backButton: { margin: 10, marginTop: 0, paddingVertical: 13, borderRadius: 11, backgroundColor: '#314b38', alignItems: 'center', borderWidth: 2, borderColor: '#203125' }, backText: { color: '#fff', fontWeight: '900', fontSize: 11, letterSpacing: 0.6 },
});
