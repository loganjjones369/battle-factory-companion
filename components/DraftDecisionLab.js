...
const statusLabel = (status) => status === 'paralyzed' ? 'PARALYZED' : 'HEALTHY';
...
export default function DraftDecisionLab({ draft = [], blockedSpecies = [], scientist = {}, levelMode = 'Open Level', battle = 1, revealed = {}, noland = false }) {
  ...
  const toggleParalysis = (p) => {
    const k = key(p);
    setAnalysis(null);
    setStatuses((old) => ({ ...old, [k]: old[k] === 'paralyzed' ? 'healthy' : 'paralyzed' }));
  };
  ...
  return <View style={st.card}>
    ...
    <Text style={st.help}>Pick any three from the six, then mark a Pokémon PARALYZED if you want to test the post-Thunder-Wave reality. That status stays attached to this scenario until you turn it off.</Text>

    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={st.row}>
      {usable.map((p) => { const on = selectedKeys.includes(key(p)); const paralyzed = statuses[key(p)] === 'paralyzed'; return <View key={key(p)} style={st.slot}><TouchableOpacity onPress={() => toggle(p)} style={[st.pokemon, on && st.pokemonOn]}><Text style={st.pokeName}>{p.species}</Text><Text style={st.pokeSet}>SET {p.setId}{p.isElevated ? ' • ↑' : ''}</Text><Text style={st.pokeItem}>{p.item || 'Item unknown'}</Text>{on && <Text style={st.check}>✓ IN LINEUP</Text>}</TouchableOpacity>{on && <TouchableOpacity onPress={() => toggleParalysis(p)} style={[st.statusButton, paralyzed && st.statusOn]}><Text style={st.statusText}>{paralyzed ? '⚡ PARALYZED' : 'SET PARALYSIS'}</Text></TouchableOpacity>}</View>; })}
    </ScrollView>
    ...
    {!!saved.length && <View style={st.saved}><Text style={st.small}>YOUR ALTERNATIVES</Text>{saved.map((s) => <TouchableOpacity key={s.id} onPress={() => loadScenario(s)} style={st.savedRow}><Text style={st.savedLabel}>{s.label}</Text><Text style={st.savedNames}>{s.names.join(' • ')}{Object.values(s.statuses || {}).includes('paralyzed') ? ' • paralysis test' : ''}</Text></TouchableOpacity>)}</View>}
    ...
    {analysis && <View style={st.results}>
      ...
      <View style={st.statusSummary}>{analysis.lineup.map((p) => { const k = key(p); const s = analysis.statuses[k] || 'healthy'; const raw = getStats(p, p, levelNumber, round).spe; const effective = getEffectiveSpeed({ spe: raw }, s); return <View key={k} style={st.statusRow}><Text style={st.statusName}>{p.species} {p.setId}</Text><Text style={st.speedText}>{statusLabel(s)} • {effective} Speed{s === 'paralyzed' ? ` (normal ${raw})` : ''}</Text></View>; })}</View>
      ...
      <Text style={st.note}>Paralysis changes Speed for this scenario only. Poison, burn and sleep are not treated as Speed modifiers here. Damage calculations continue to use the Pokémon's normal stats unless a status specifically changes damage.</Text>
...
const st = StyleSheet.create({
  ...
  statusButton:{marginTop:4,minHeight:28,borderWidth:1,borderColor:'#304943',borderRadius:8,paddingVertical:6,alignItems:'center',justifyContent:'center'}, statusOn:{borderColor:'#d0b34e',backgroundColor:'#302c17'}, statusText:{color:'#a9c2ba',fontSize:7,fontWeight:'900'},
