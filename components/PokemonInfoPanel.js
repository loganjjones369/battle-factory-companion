...
function SummaryPage({ page, pokemon, level, round }) {
  ...
  const sets = selectedSet ? [selectedSet] : possibleSets(pokemon).filter(set => pokemon?.poolBucket ? String(set?.pool || '') === String(pokemon.poolBucket) : true);
  ...
  return <View style={[styles.page, palette]}>
    ...
    {page === 1 && <><Text style={styles.pageTitle}>MOVES</Text>{(selectedSet?.moves || pokemon?.moves || ['???','???','???','???']).map((move, i) => <View key={i} style={styles.moveRow}><Text style={styles.moveName}>{move || '???'}</Text><Text style={styles.qmark}>{move ? 'KNOWN' : '???'}</Text></View>)}<Text style={styles.pageNote}>Factory move order is preserved exactly.</Text></>}
    ...
    {page === 3 && <><Text style={styles.pageTitle}>INDIVIDUAL VALUES</Text><Text style={styles.pageNote}>{pokemon?.factoryIV != null ? `${pokemon.factoryIVSource || 'Factory source'} • fixed IVs: ${pokemon.factoryIV} in every stat` : 'IVs are unknown until the Factory source is known.'}</Text>{['hp','atk','def','spa','spd','spe'].map((key) => <View key={key} style={styles.statRow}><Text style={styles.statName}>{key.toUpperCase()}</Text><Text style={styles.statValue}>{selectedSet?.ivs?.[key] ?? pokemon?.factoryIV ?? '???'}</Text></View>)}</>}
