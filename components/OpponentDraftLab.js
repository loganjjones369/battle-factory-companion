...
function speedOf(p, level, round) {
  return getEffectiveSpeed(getStats(p, p, level, round), 'healthy');
}
...
function inspectOpponentTeam(playerTeam, opponentTeam, level, round) {
  const rows = opponentTeam.map((foe) => {
    const foeSpeed = speedOf(foe, level, round);
    const answers = playerTeam.map((ally) => {
      const hit = bestHit(foe, ally, level, round);
      const faster = foeSpeed > speedOf(ally, level, round);
      return { ally, hit, faster };
    });
    const dangerous = answers.filter((x) => x.faster || x.hit?.percentMax >= 50 || (x.hit?.ko && x.hit.ko <= 2));
    return { foe, answers, dangerous };
  });
  const hardThreats = rows.filter((r) => r.dangerous.some((x) => x.hit?.ko && x.hit.ko <= 2));
  const speedThreats = rows.filter((r) => r.dangerous.some((x) => x.faster));
  return { rows, hardThreats, speedThreats };
}

export function analyzeOpponentDraft({ draft = [], scientist = {}, levelMode = 'Open Level', battle = 1, blockedSpecies = [], revealed = {}, currentTeam = [], previousOpponent = [], noland = false } = {}) {
  ...
  // Screen actual clue-compatible legal Factory teams. Frequency is not treated as probability.
  ...
  for (let a = 0; a < team.length; a += 1) {
    for (let b = a + 1; b < team.length; b += 1) {
      for (let c = b + 1; c < team.length; c += 1) {
        const playerTeam = [team[a], team[b], team[c]];
        let dangerousTeams = 0;
        let cleanTeams = 0;
        let hardTeams = 0;
        let speedTeams = 0;
        const examples = [];
        screened.forEach((opponentTeam) => {
          const inspected = inspectOpponentTeam(playerTeam, opponentTeam, level, round);
          ...
          if (examples.length < 4 && dangerous) {
            const row = inspected.rows.find((r) => r.dangerous.length);
            const hit = row?.dangerous[0];
            if (row && hit) examples.push({ foe: row.foe, ally: hit.ally, hit: hit.hit, faster: hit.faster });
          }
        });
...
export default function OpponentDraftLab({ draft = [], scientist = {}, levelMode = 'Open Level', battle = 1, blockedSpecies = [], revealed = {}, currentTeam = [], previousOpponent = [], noland = false }) {
  const [open, setOpen] = useState(null);
  const result = useMemo(() => analyzeOpponentDraft({ draft, scientist, levelMode, battle, blockedSpecies, revealed, currentTeam, previousOpponent, noland }), [draft, scientist, levelMode, battle, blockedSpecies, revealed, currentTeam, previousOpponent, noland]);
  ...
  return <View style={st.card}>
    ...
    <Text style={st.help}>This screens clue-compatible Factory teams directly. The percentages describe the screened candidate set, not an in-game probability.</Text>
    {result.rows.map((row, index) => {
      const openRow = open === index;
      const label = row.combo.map((p) => `${p.species} ${p.setId ?? ''}`).join(' • ');
      return <View key={label} style={st.row}>
        <TouchableOpacity onPress={() => setOpen(openRow ? null : index)} style={st.rowButton}>
          <View style={{ flex: 1 }}><Text style={st.rank}>#{index + 1}  {label}</Text><Text style={st.meta}>{Math.round(row.dangerousShare * 100)}% screened teams contain a direct pressure point • {Math.round(row.hardShare * 100)}% contain a 2HKO-or-better threat • {Math.round(row.speedShare * 100)}% contain a faster threat</Text></View><Text style={st.chevron}>{openRow ? '▲' : '▼'}</Text>
        </TouchableOpacity>
        {openRow && <View style={st.details}><Text style={st.detailTitle}>WHAT THE SCREEN FOUND</Text><Text style={st.detail}>{row.cleanTeams}/{row.screenedTeams} screened teams produced no high-priority pressure under the current damage map.</Text>{row.examples.map((x, i) => <View key={`${key(x.foe)}-${i}`} style={st.example}><Text style={st.exampleTitle}>{x.foe.species} {x.foe.setId ?? ''} → {x.ally.species} {x.ally.setId ?? ''}</Text><Text style={st.exampleText}>{x.hit ? `${x.hit.moveName}: ${x.hit.percentMin.toFixed(1)}–${x.hit.percentMax.toFixed(1)}%` : 'No mapped damage'}{x.faster ? ' • faster' : ''}</Text></View>)}</View>}
      </View>;
    })}
  </View>;
}
...
const st = StyleSheet.create({
  ...
  title:{color:'#f2f7f4',fontSize:17,fontWeight:'900',marginTop:3},
  ...
  help:{color:'#a9c0ba',fontSize:11,lineHeight:17,marginBottom:9},
  empty:{color:'#d8b98c',fontSize:12,lineHeight:18,marginTop:7},
  row:{borderTopWidth:1,borderTopColor:'#27423a'},
  rowButton:{minHeight:66,flexDirection:'row',alignItems:'center',paddingVertical:9},
  rank:{color:'#edf6f2',fontSize:12.5,fontWeight:'800'},
  meta:{color:'#87aaa1',fontSize:10.5,lineHeight:15,marginTop:3},
  chevron:{color:'#8bd3bb',fontSize:13,paddingHorizontal:7},
  details:{backgroundColor:'#0c1a17',borderRadius:12,padding:10,marginBottom:10},
  detailTitle:{color:'#9fe2c8',fontSize:10,fontWeight:'900',letterSpacing:.7,marginBottom:5},
  detail:{color:'#c7d8d3',fontSize:10.5,lineHeight:16},
  example:{marginTop:8,paddingTop:7,borderTopWidth:1,borderTopColor:'#213a34'},
  exampleTitle:{color:'#f0f7f4',fontSize:11,fontWeight:'800'},
  exampleText:{color:'#8bd3bb',fontSize:10.5,marginTop:2},
});
