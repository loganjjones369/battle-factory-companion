...
export default function LiveOpponentAnalysis({ draft = [], scientist = {}, levelMode = 'Open Level', battle = 1, blockedSpecies = [], observations = [], currentTeam = [], previousOpponent = [], noland = false }) {
  ...
  return <View style={styles.card}>
    ...
    {!result.supported ? <Text style={styles.warning}>{result.reason}</Text> : <>
      ...
      {!!uncertainty.bestNextClues?.length && <View style={styles.nextClueBox}><Text style={styles.nextClueTitle}>BEST NEXT CLUES TO WATCH FOR</Text><Text style={styles.nextClueSub}>These are the clues with the biggest reduction among the surviving legal team combinations.</Text>{uncertainty.bestNextClues.slice(0, 5).map((clue, index) => <View key={`${clue.kind}-${clue.species || ''}-${clue.value}`} style={styles.nextClueRow}><Text style={styles.nextClueRank}>{index + 1}</Text><Text style={styles.nextClueText}>If you see {clue.kind === 'move' ? `the move ${clue.value}` : `the item ${clue.value}`} on {clue.species || 'that Pokémon'}, {clue.remainingTeams != null ? `${clue.remainingTeams} candidate team combinations remain${clue.eliminatedTeams ? `; ${clue.eliminatedTeams} are eliminated` : ''}.` : `${clue.remaining} candidate set${clue.remaining === 1 ? '' : 's'} remain${clue.remaining === 1 ? 's' : ''} among ${(clue.species || []).join(' / ')}.`}</Text></View>)}</View>}
