...
function speedSummary(attacker, defender, level, round, attackerStages, defenderStages, attackerStatus, defenderStatus) {
  if (!attacker?.species || !defender?.species) return null;
  const aSets = setPool(attacker), dSets = setPool(defender);
  const speedOf = (p, stages, status) => getEffectiveSpeed(applyStatStages(getStats(p, p, level, round), stages), status);
  const rows = aSets.flatMap((a) => dSets.map((d) => { const aSpeed=speedOf(a,attackerStages,attackerStatus), dSpeed=speedOf(d,defenderStages,defenderStatus); return {a,d,aSpeed,dSpeed,faster:aSpeed>dSpeed,tie:aSpeed===dSpeed}; }));
  const faster=rows.filter(r=>r.faster).length, ties=rows.filter(r=>r.tie).length, total=rows.length;
  const attackerWinningSets=aSets.filter(a=>dSets.every(d=>speedOf(a,attackerStages,attackerStatus)>speedOf(d,defenderStages,defenderStatus))).length;
  return {aSets,dSets,rows,faster,ties,total,attackerWinningSets};
}
function moveRows(attacker, defender, level, round, attackerStages, defenderStages, attackerStatus, defenderStatus) {
  const a=makeKnownSet(attacker), d=makeKnownSet(defender); if(!a||!d)return [];
  return bestDamagingMoves(a).map(moveName=>({moveName,result:calculateDamage({attacker:a,attackerSet:a,defender:d,defenderSet:d,level,round,moveName,attackerStages,defenderStages,attackerStatus,defenderStatus})}));
}
...
export default function DraftCalculatorV2({ draft=[], level='Level 50', battle=1 }) {
  ...
  const [attackerStatus,setAttackerStatus]=useState('healthy'); const [defenderStatus,setDefenderStatus]=useState('healthy');
  ...
  const speed=useMemo(()=>speedSummary(attacker,defender,levelNumber,round,attackerStages,defenderStages,attackerStatus,defenderStatus),[attacker,defender,levelNumber,round,attackerStages,defenderStages,attackerStatus,defenderStatus]);
  const moves=useMemo(()=>moveRows(attacker,defender,levelNumber,round,attackerStages,defenderStages,attackerStatus,defenderStatus),[attacker,defender,levelNumber,round,attackerStages,defenderStages,attackerStatus,defenderStatus]);
  return <View style={st.card}>
    ...
    <Text style={st.help}>Choose two Pokémon from the six-draft screen. Stat stages and paralysis are applied to both damage and Speed analysis.</Text>
    <Text style={st.small}>ATTACKER</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={st.horizontal}>{usable.map(p=><SetButton key={`a-${p.species}-${p.setId}`} p={p} selected={attackerKey===`${norm(p.species)}#${p.setId}`} onPress={()=>setAttackerKey(`${norm(p.species)}#${p.setId}`)}/>)}</ScrollView>
    <Text style={st.small}>DEFENDER</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={st.horizontal}>{usable.map(p=><SetButton key={`d-${p.species}-${p.setId}`} p={p} selected={defenderKey===`${norm(p.species)}#${p.setId}`} onPress={()=>setDefenderKey(`${norm(p.species)}#${p.setId}`)}/>)}</ScrollView>
    {attacker&&defender&&<>
