...
function Slot({ pokemon, side, index, active, knockedOut, onPress }) {
  return <TouchableOpacity onPress={onPress} activeOpacity={.85} style={[styles.slot, side === 'you' ? styles.youSlot : styles.foeSlot, active && styles.activeSlot, knockedOut && styles.koSlot]}>
    ...
    {active && <View style={styles.arenaLight} />}
...
export default function BattleRoom({
  ...
  const activeYou = team[activeTeamIndex];
  const activeFoe = opponent[activeOpponentIndex];
  ...
  return <View style={styles.room}>
    ...
    <View style={styles.arena}>
      ...
      <View style={styles.foeRail}>
        {[0,1,2].map(i => <Slot key={i} pokemon={opponent[i]} side="foe" index={i} active={activeOpponentIndex === i && !!opponent[i]} knockedOut={knockedOut.opponent.includes(i)} onPress={() => opponent[i] && onSelectOpponent?.(i)} />)}
      </View>
      ...
      <View style={styles.youRail}>
        {[0,1,2].map(i => <Slot key={i} pokemon={team[i]} side="you" index={i} active={activeTeamIndex === i && !!team[i]} knockedOut={knockedOut.team.includes(i)} onPress={() => team[i] && onSelectTeam?.(i)} />)}
      </View>
      ...
      <View style={styles.quick}>
        ...
        <Text style={styles.quickHint}>{activeFoe ? 'Damage / sets / speed update from the scenario instrument below.' : 'Enter the first opponent Pokémon to begin.'}</Text>
        <View style={styles.quickActions}>
          <TouchableOpacity onPress={() => onOpenSummary?.(activeYou, 'YOUR POKÉMON')} style={styles.quickButton}><Text style={styles.quickButtonText}>SUMMARY</Text></TouchableOpacity>
          <TouchableOpacity onPress={() => onOpenSummary?.(activeFoe, 'OPPONENT')} disabled={!activeFoe} style={[styles.quickButton, !activeFoe && styles.disabled]}><Text style={styles.quickButtonText}>FOE INFO</Text></TouchableOpacity>
        </View>
      </View>
