...
function QuickReference({ source, target, side, setId, setProbabilities = {}, onSet, onObserveMove, onObserveItem, onObserveAbility, scenario, onScenarioChange }) {
  ...
  const setList = getFactorySets(source.species).map(s=>s.id).filter(id => setProbabilities?.[id] == null || Number(setProbabilities[id]) > 0);
  ...
  return <View style={styles.quick}>
    ...
    {side==='opponent' && <View style={styles.observeBox}><Text style={styles.observeTitle}>OBSERVED</Text><Text style={styles.observeHint}>Tap a clue as soon as you see it. It immediately narrows the remaining sets.</Text><View style={styles.observeRows}><Text style={styles.observeLabel}>MOVE</Text>{(set.moves||[]).slice(0,4).map(move=><TouchableOpacity key={move} onPress={()=>onObserveMove?.(source._index, move)} style={styles.observeChip}><Text style={styles.observeChipText}>{move}</Text></TouchableOpacity>)}</View><TouchableOpacity onPress={()=>onObserveItem?.(source._index,set.item)} style={styles.observeButton}><Text style={styles.observeButtonText}>OBSERVE ITEM · {set.item || 'UNKNOWN'}</Text></TouchableOpacity><TouchableOpacity onPress={()=>onObserveAbility?.(source._index,set.ability)} style={styles.observeButton}><Text style={styles.observeButtonText}>OBSERVE ABILITY · {set.ability || 'UNKNOWN'}</Text></TouchableOpacity></View>}
