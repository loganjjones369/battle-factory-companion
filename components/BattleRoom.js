...
function QuickReference({ source, target, side, setId, setProbabilities = {}, onSet, onConfirmSet, onObserveMove, onObserveItem, onObserveAbility, onClearClue, observedAbility, observedMoves = [], observedItem = '', observedDamagePercent = null, observedDamageMove = '', observedSpeed = null, observedSpeedRelation = '', scenario, onScenarioChange, onRecordObservedDamage, onClearObservedDamage }) {
  ...
  return <View style={styles.quick}>
    ...
    {side==='opponent' && <View style={styles.observeBox}><Text style={styles.observeTitle}>OBSERVED</Text><View style={styles.observeDamageRow}><TextInput value={String(observedDamagePercent ?? '')} onChangeText={(v)=>onRecordObservedDamage?.(source._index, observedDamageMove || '', v)} placeholder="Damage %" keyboardType="decimal-pad" style={styles.damageInput}/><Text style={styles.observeHint}>Enter the % lost, then tap the move that caused it.</Text></View><View style={styles.observeDamageRow}><TextInput value={observedSpeed == null ? '' : String(observedSpeed)} onChangeText={(v)=>onRecordObservedSpeed?.(source._index,v,observedSpeedRelation||'equal')} placeholder="Observed Speed" keyboardType="number-pad" style={styles.damageInput}/><TouchableOpacity onPress={()=>onRecordObservedSpeed?.(source._index,observedSpeed,'equal')} style={[styles.observeChip,observedSpeedRelation==='equal'&&styles.observeChipOn]}><Text style={styles.observeChipText}>=</Text></TouchableOpacity><TouchableOpacity onPress={()=>onRecordObservedSpeed?.(source._index,observedSpeed,'at least')} style={[styles.observeChip,observedSpeedRelation==='at least'&&styles.observeChipOn]}><Text style={styles.observeChipText}>≥</Text></TouchableOpacity><TouchableOpacity onPress={()=>onRecordObservedSpeed?.(source._index,observedSpeed,'at most')} style={[styles.observeChip,observedSpeedRelation==='at most'&&styles.observeChipOn]}><Text style={styles.observeChipText}>≤</Text></TouchableOpacity></View></View><Text style={styles.observeHint}>Tap a clue as soon as you see it. It immediately narrows the remaining sets.</Text><View style={styles.observeRows}><Text style={styles.observeLabel}>MOVE</Text>{[...new Set(getFactorySets(source.species).filter(s=>validSetIds.includes(s.id)).flatMap(s=>s.moves||[]))].map(move=><TouchableOpacity key={move} onPress={()=>{if(scenario?.observedDamagePercent != null){onRecordObservedDamage?.(source._index,move,scenario.observedDamagePercent);} else {observedMoves.includes(move)?onClearClue?.(source._index,'move',move):onObserveMove?.(source._index, move);}}} style={[styles.observeChip,observedMoves.includes(move)&&styles.observeChipOn]}><Text style={styles.observeChipText}>{observedMoves.includes(move)?'✓ ':''}{move}</Text></TouchableOpacity>)}</View><View style={styles.observeRows}><Text style={styles.observeLabel}>ITEM</Text>{[...new Set(getFactorySets(source.species).filter(s=>validSetIds.includes(s.id)).map(s=>s.item).filter(Boolean))].map(item=><TouchableOpacity key={item} onPress={()=>observedItem===item?onClearClue?.(source._index,'item',item):onObserveItem?.(source._index,item)} style={[styles.observeChip,observedItem===item&&styles.observeChipOn]}><Text style={styles.observeChipText}>{observedItem===item?'✓ ':''}{item}</Text></TouchableOpacity>)}</View>{side==='opponent' && <View style={styles.abilityChoices}><Text style={styles.observeLabel}>ABILITY</Text>{[...new Set(getFactorySets(source.species).filter(s=>setProbabilities?.[s.id] == null || Number(setProbabilities[s.id]) > 0).flatMap(s=>String(s.ability||'').split('/').map(a=>a.trim()).filter(Boolean)))].map(a=><TouchableOpacity key={a} onPress={()=>onObserveAbility?.(source._index,a)} style={[styles.observeChip, observedAbility===a&&styles.observeChipOn]}><Text style={styles.observeChipText}>{a}</Text></TouchableOpacity>)}</View>}</View>}
    ...
    {editingPP && <View style={styles.ppEdit}><Text style={styles.ppTitle}>EDIT PP · {editingPP}</Text><TextInput keyboardType="number-pad" value={String(pp[editingPP] ?? 4)} onChangeText={v=>{const n=Math.max(0,Math.min(99,parseInt(v||'0',10)||0));onScenarioChange?.({...scenario,pp:{...(scenario.pp||{}),[ppKey]:{...pp,[editingPP]:n}}})}} style={styles.searchInput}/><TouchableOpacity onPress={()=>setEditingPP(null)}><Text style={styles.rollClose}>DONE</Text></TouchableOpacity></View>}    {rolls && <View style={styles.rollPanel}><Text style={styles.rollTitle}>{rolls} · DAMAGE ROLLS</Text>{(()=>{const rr=rows.find(r=>r.move===rolls)?.result;const vals=getDamageRolls(rr);const dist=getDamageRollDistribution(rr);const hp=targetSet?getStats(target,targetSet,level,1).hp:1;const pct=vals.map(v=>(v*100/hp).toFixed(1));return <><Text style={styles.rollRange}>{rr?.percentMin ?? 0}–{rr?.percentMax ?? 0}% · 39 RANDOM ROLLS · {vals.length} POSSIBLE VALUES</Text><Text style={styles.rollHint}>{dist.map(d=>`${(d.damage*100/hp).toFixed(1)}% × ${d.rolls}`).join(' · ')}</Text></>})()}<TouchableOpacity onPress={()=>setRolls(null)}><Text style={styles.rollClose}>× CLOSE</Text></TouchableOpacity></View>}  </View>;
}
export default function BattleRoom({
  ...
  observedAbilities = [],
  observedItems = [],
  observedDamagePercents = [],
  observedDamageMoves = [],
  observedSpeeds = [],
  observedSpeedRelations = [],
}) {
  ...
  return <View style={styles.room}>
    ...
    <View style={styles.arena}>
      ...
      <View style={styles.statusArea}>        <Text style={styles.hpText}>{viewedSide==='team' ? 'YOU' : 'FOE'} HP {viewedSide==='team' ? (scenario?.hp?.['team:'+activeTeamIndex] ?? '—') : (scenario?.hp?.['opponent:'+activeOpponentIndex] ?? '—')} / {viewedSide==='team' ? (activeYou ? getStats(activeYou, getFactorySet(activeYou.species, activeYou.setId), activeYou.level || 50, 1).hp : '—') : (activeFoe && setId != null ? getStats(activeFoe, getFactorySet(activeFoe.species, setId), activeFoe.level || 50, 1).hp : '—')}</Text>
        <TouchableOpacity onPress={() => setStatusOpen(v => !v)} style={styles.statusButton}><Text style={styles.statusText}>[{viewedSide==='team' ? statusLabel : String(foeStatus).toUpperCase()}]</Text></TouchableOpacity>
        {statusOpen && <View style={styles.statusMenu}>{STATUS.map(s => <TouchableOpacity key={s} onPress={() => { const key=(viewedSide==='team'?'team':'opponent')+':'+(viewedSide==='team'?activeTeamIndex:activeOpponentIndex); onScenarioChange?.({ ...scenario, status: { ...(scenario.status || {}), [key]: s.toLowerCase() } }); setStatusOpen(false); }} style={styles.statusOption}><Text style={styles.statusOptionText}>{s}</Text></TouchableOpacity>)}</View>}
