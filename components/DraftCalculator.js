...
import { abilityDamageEffect, abilityOptions, abilitySpeedMultiplier, abilitySummary } from '../data/abilityCalc';
...
function moveRows(attacker, defender, level, round, attackerStages, defenderStages, attackerStatus, defenderStatus, attackerAbility, defenderAbility) {
  const a=makeKnownSet(attacker), d=makeKnownSet(defender); if(!a||!d)return [];
  return bestDamagingMoves(a).map(moveName=>{
    const result=calculateDamage({attacker:a,attackerSet:a,defender:d,defenderSet:d,level,round,moveName,attackerStatus,defenderStatus,attackerStages,defenderStages});
    const effect=abilityDamageEffect({moveType:result.unsupported?null:requireMoveType(moveName),effectiveness:result.effectiveness,category:requireMoveCategory(moveName),attackerAbility,defenderAbility});
    if(effect.immune)return {moveName,result:{...result,min:0,max:0,percentMin:0,percentMax:0,ko:null,abilityNote:effect.reason}};
    if(effect.multiplier!==1){
      const min=Math.floor(result.min*effect.multiplier), max=Math.floor(result.max*effect.multiplier);
      return {moveName,result:{...result,min,max,percentMin:Math.floor(min*100/result.hp*10)/10,percentMax:Math.floor(max*100/result.hp*10)/10,ko:Math.ceil(result.hp/Math.max(1,min)),abilityNote:effect.reason}};
    }
    return {moveName,result};
  });
}

// Kept local so the UI can remain independent of MOVE_DATA internals.
import { MOVE_DATA } from '../data/damageCalc';
const requireMoveType = (move) => MOVE_DATA[move]?.type;
const requireMoveCategory = (move) => MOVE_DATA[move]?.category;
...
export default function DraftCalculator({ draft=[], level='Level 50', battle=1 }) {
  ...
  const moves=useMemo(()=>moveRows(attacker,defender,levelNumber,round,attackerStages,defenderStages,attackerStatus,defenderStatus,atkAbility,defAbility),[attacker,defender,levelNumber,round,attackerStages,defenderStages,attackerStatus,defenderStatus,atkAbility,defAbility]);
  return <View style={st.card}>
    ...
    {attacker&&defender&&<>
      <View style={st.weatherBox}><Text style={st.small}>WEATHER</Text><View style={st.statusRow}>{['none','sun','rain'].map(w=><StatusButton key={w} label={w==='none'?'CLEAR':w.toUpperCase()} active={weather===w} onPress={()=>setWeather(w)}/>)}</View></View>
