...
function speedSummary(attacker, defender, level, round, attackerStages = DEFAULT_STAT_STAGES, defenderStages = DEFAULT_STAT_STAGES, attackerStatus = 'healthy', defenderStatus = 'healthy') {
  ...
  const rows = aSets.flatMap((a) => dSets.map((d) => {
    const aSpeed = speedOf(a, attackerStages, attackerStatus);
    const dSpeed = speedOf(d, defenderStages, defenderStatus);
  ...
  const defenderWinningSets = dSets.filter((d) => aSets.every((a) => speedOf(d, defenderStages, defenderStatus) > speedOf(a, attackerStages, attackerStatus))).length;
...
function StatusButton({ label: text, active, onPress }) {
  return <TouchableOpacity onPress={onPress} style={[st.statusButton, active && st.statusOn]}><Text style={st.statusText}>{text}</Text></TouchableOpacity>;
}
...
export default function DraftCalculator({ draft = [], level = 'Level 50', battle = 1 }) {
  ...
  const [attackerStages, setAttackerStages] = useState({ ...DEFAULT_STAT_STAGES });
  const [defenderStages, setDefenderStages] = useState({ ...DEFAULT_STAT_STAGES });
  ...
  return <View style={st.card}>
    ...
    <Text style={st.help}>Pick any two Pokémon from the six-draft screen. Stat stages and status effects are applied to both damage and Speed analysis.</Text>

    <Text style={st.small}>ATTACKER</Text>
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={st.horizontal}>{usable.map((p) => <SetButton key={`a-${p.species}-${p.setId}`} p={p} selected={attackerKey === `${norm(p.species)}#${p.setId}`} onPress={() => setAttackerKey(`${norm(p.species)}#${p.setId}`)} />)}</ScrollView>
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={st.horizontal}>{usable.map((p) => <SetButton key={`d-${p.species}-${p.setId}`} p={p} selected={defenderKey === `${norm(p.species)}#${p.setId}`} onPress={() => setDefenderKey(`${norm(p.species)}#${p.setId}`)} />)}</ScrollView>
...
const st = StyleSheet.create({
  ...
  stateCard:{borderWidth:1,borderColor:'#243a34',borderRadius:12,backgroundColor:'#0a1512',padding:8,marginTop:8},
  stateTitle:{color:'#b8d5cc',fontSize:8,fontWeight:'900',letterSpacing:.7},
  statusRow:{flexDirection:'row',flexWrap:'wrap',gap:5,marginTop:7},
  statusButton:{borderWidth:1,borderColor:'#29403a',borderRadius:7,paddingHorizontal:8,paddingVertical:5,backgroundColor:'#0d1b17'},
  statusOn:{borderColor:'#70d4af',backgroundColor:'#17382f'},
  statusText:{color:'#8ca69e',fontSize:7,fontWeight:'900'},
