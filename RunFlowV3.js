...
export default function RunFlowV3(){
 ...
 <DraftCalculator draft={draft} level={level} battle={b}/><CandidateAnalysisPanel draft={draft} blockedSpecies={draft.filter(Boolean).map(p=>p.species)} scientist={state?.scientist||{}} levelMode={level} battle={b} revealed={state?.revealed||{}} noland={state?.noland||false}/></>:phase==='battle'?<>
 <BattleRoom
   team={team}
   opponent={opponent}
